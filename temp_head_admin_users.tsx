...
export default function AdminUsersPage() {
  ...
  // Permiss├Áes do utilizador atual
  useEffect(() => {
  ...
  // Stats r├ípidas (v├¬m de /api/admin/stats -> users block)
  useEffect(() => {
  ...
  // Op├º├Áes de filtros (pa├¡s e desporto)
  ...
  const renderSortIcon = (key: SortKey) => {
    if (sortKey !== key)
      return <span className="ml-1 text-xs text-gray-400">Ôåò</span>;
    return (
  ...
  if (loading || !permissionsLoaded) {
    return (
      <div className="w-full">
        <p className="text-sm text-blue-100/90">A carregar utilizadoresÔÇª</p>
      </div>
    );
  }
      ...
      <section className="mt-2 rounded-2xl border border-slate-800/80 bg-gradient-to-br from-slate-950 via-slate-900 to-slate-950 relative overflow-hidden px-4 py-6 md:px-6 md:py-8">
        ...
        <div className="relative z-10 max-w-5xl">
          ...
          <p className="mt-2 text-sm md:text-base text-blue-100/90 max-w-2xl">
            Gest├úo centralizada de roles, permiss├Áes e filtros de utilizadores
            por pa├¡s, desporto, atividade recente e XP. A sala de controlo
          ...
          <Card className="bg-card-custom border-custom shadow-lg shadow-emerald-950/40">
            <CardHeader>
              ...
              <p className="text-sm text-muted-custom">
                Filtros avan├ºados por role, pa├¡s, desporto, ├║ltimo login e
                ├║ltimo XP.
              </p>
            </CardHeader>
                      ...
                      filteredAndSortedUsers.map((u) => (
                        ...
                        >
                          ...
                          <td className="px-4 py-2 text-xs">
                            {u.last_login ? formatDate(u.last_login) : 'ÔÇö'}
                          </td>
                          <td className="px-4 py-2 text-xs">
                            {u.last_xp_at ? formatDate(u.last_xp_at) : 'ÔÇö'}
                          </td>
                          <td className="px-4 py-2">
                            {canEditUsers ? (
                              <Select
