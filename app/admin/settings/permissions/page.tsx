...
export default function AdminPermissionsPage() {
  ...
  useEffect(() => {
    ...
    if (!isSuperAdmin) {
      // Apenas Super Admin vê esta página
      router.push('/admin');
      return;
    }
  ...
  const handleToggle = (userId: string, perm: Permission) => {
    setAdmins((prev) =>
      prev.map((a) => {
        ...
        // Permissões base não podem ser removidas
          ...
          <div className="mb-6 flex items-center justify-between gap-4">
            <div>
              ...
              <p className="text-gray-600 text-sm">
                Grant or revoke specific admin capabilities. Only Super Admins
          ...
          <Card className="mb-4">
            <CardHeader>
              ...
              <CardDescription className="text-sm">
                Super Admin has full access by default. Admins start with basic
                access and can be given extra permissions here. Head of House
                powers são tratadas à parte, por House.
              </CardDescription>
            </CardHeader>
          </Card>
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                {loadingData
                  ? 'Loading admins...'
                  : `Showing ${admins.length} admin user(s).`}
              </CardDescription>
            </CardHeader>
            <CardContent>
              {loadingData ? (
                            ...
                            <td className="py-2 px-3">
                              {isSuper ? (
                                <p className="text-xs text-gray-500">
                                  Super Admin has full access to all
                                  permissions.
                                </p>
                              ) : (
                                <div className="flex flex-wrap gap-2">
                                  {sortedPermissions.map((perm) => {
                                    const isBase =
                                      admin.basePermissions.includes(perm);
                                    const isChecked =
                                      isBase ||
