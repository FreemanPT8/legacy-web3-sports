...
- Fundamentos: 1 pop-up = 1 decisao util, maximo 1 por dia e 3 por semana por utilizador, mensagem nunca parece venda e CTA principal e sempre opcional.
...
4. **Governanca e auditoria**
   ...
   - Alertas automaticos disparam se uma House ultrapassar limites, tentar remover o bloqueio ou publicar sem revisao.
...
3. **Definir trigger**: XP, evento de conteudo ou ambos, limites extras (ex.: "so uma vez por trimestre"), dependencias e prioridade.
...
7. **Sport Pools**: novo painel (`/admin/houses/pools`) centraliza contas que aguardam desporto/House (sem desporto, desporto ainda sem House, ou sugestao de novo desporto). Inclui filtros por estado, desporto e pais, pesquisa por email e refresco manual. Ao atribuir/remover um utilizador, o sistema atualiza automaticamente `user_houses`, envia notificacoes internas para Heads e Super Admins e marca essas notificacoes como tratadas assim que a entrada for resolvida.
...
- Se o limite diario/semanal foi atingido, novos triggers entram em fila ("mensagens pendentes") e sao entregues assim que abrir nova janela.
...
- **Centro de conformidade**: checklist semanal confirma se o Head manteve limites, assinou Termo nos ultimos 90 dias e nao teve incidentes de abuso; em caso negativo, bloqueia novas publicacoes.
...
3. Construir middleware de exibicao com limites, bloqueio, fila e logging completo.
4. Implementar auditoria automatizada (validar Termo, limites, idiomas, templates) e canal de alertas para Operacoes.
5. Publicar guia para Heads com sequencias exemplo, boas praticas e checklists de conformidade.
6. Medir piloto com 1-2 Houses antes de abrir para todas; ajustar triggers, limites e UX conforme dados.
...
- `popup_triggers`: popup_id, tipo (`xp`, `conteudo`, `combinado`), xp_min, xp_max, conteudo_id, dependencia_popup_id, limites_extra.
...
- `head_audits`: head_user_id, termo_aceite_em, violacao_limite (bool), detalhes, resolvido_em.

## Roadmap de Implementacao
1. **Descoberta & alinhamento (Semana 0-1)**
   - Workshops com Heads e produto para confirmar casos de uso, triggers e limites.
...
3. **Motor e camada de exibicao (Semana 3-6)**
   - Construir serviço que escuta eventos de XP e conteudo, avalia triggers e agenda pop-ups respeitando limites.
...
- **Alertas automáticos**: e-mails/slack para Operações quando limites são atingidos, quando um Head tenta remover bloqueio 3 s ou quando uma House faz mais de 2 broadcasts no mês.
...
## Testes e Qualidade
- **Testes unitários** para motor de triggers (XP e eventos), cálculo de limites e fila.
- **Testes E2E** simulando utilizadores em diferentes casas/idiomas, garantindo bloqueio 3 s e CTAs opcionais.
- **Testes de carga** no motor de eventos para assegurar que milhares de triggers simultâneos respeitam limites e não duplicam pop-ups.
